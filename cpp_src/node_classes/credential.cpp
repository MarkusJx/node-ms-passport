...
#define CHECK_VALID() if (!valid) throw Napi::Error::New(info.Env(), "This credential instance is not valid")
...
public:
    credential_creator() : encrypt(false), pass(nullptr), valid(false) {}

    credential_creator(std::wstring acc, bool _encrypt, bool _valid) :
        account(std::move(acc)), encrypt(_encrypt), pass(std::make_shared<secure_wstring>()), valid(_valid) {}
    ...
    PASSPORT_UNUSED static Napi::Value toNapiValue(const Napi::Env& env, const credential_creator& c) {
...
void credential::init(Napi::Env env, Napi::Object &exports) {
    Napi::Function func = DefineClass(env, "Credential", {
        InstanceAccessor("accountId", &credential::get_account_id, nullptr, napi_enumerable),
        InstanceAccessor("username", &credential::get_username, nullptr, napi_enumerable),
        InstanceAccessor("password", &credential::get_password, nullptr, napi_enumerable),
        InstanceAccessor("encrypted", &credential::get_encrypt, nullptr, napi_enumerable),
        InstanceAccessor("passwordBuffer", &credential::get_password_buffer, nullptr, napi_enumerable),
        InstanceAccessor("valid", &credential::get_valid, nullptr, napi_enumerable),
        InstanceMethod("loadPassword", &credential::load_password, napi_enumerable),
        InstanceMethod("unloadPassword", &credential::unload_password, napi_enumerable),
        InstanceMethod("refreshData", &credential::refresh_data, napi_enumerable),
        InstanceMethod("update", &credential::update, napi_enumerable),
        InstanceMethod("setEncrypted", &credential::set_encrypted, napi_enumerable),
        InstanceMethod("isEncrypted", &credential::is_encrypted, napi_enumerable)
    });
...
Napi::Value credential::createInstance(Napi::Env env, const std::wstring &account_id, bool encrypt) {
    return napi_tools::promises::promise<credential_creator>(env, [account_id, encrypt] {
        credential_creator res(account_id, encrypt, true);
...
Napi::Value credential::enumerate(Napi::Env env, const std::shared_ptr<std::wstring> &target) {
    return napi_tools::promises::promise<std::vector<credential_creator>>(env, [target] {
        ...
        for (const auto &c : data) {
            credential_creator creator(c.target, c.encrypted, c.valid);
            creator.user = c.username;
            creator.pass = std::make_shared<secure_wstring>(c.password);
            if (!c.encrypted && c.valid && !creator.pass->empty()) {
...
credential::credential(const Napi::CallbackInfo &info) : ObjectWrap(info), mtx() {
    ...
    if (info.Length() >= 3 && info[4].IsBoolean()) {
        valid = info[4].ToBoolean();
    } else {
        // The credential is valid by default
        valid = true;
    }
}
...
Napi::Value credential::get_valid(const Napi::CallbackInfo &info) {
    return Napi::Boolean::New(info.Env(), valid);
}
...
Napi::Value credential::update(const Napi::CallbackInfo &info) {
    ...
    return napi_tools::promises::promise<void>(info.Env(), [this, u, p] {
        ...
        // We successfully wrote data, this is now valid
        valid = true;
        password_loaded = false;
    });
}
