...
array<unsigned char> ^ charToByteArray(const unsigned char *data, int len) {
    array < unsigned char > ^out = gcnew
    array<unsigned char>(len);
    for (int i = 0; i < len; i++) {
        out[i] = data[i];
    }
...
NODEMSPASSPORT_EXPORT char *passport::unmanaged::createPassportKey(int &status, int &outSize, const char *accountId) {
    CSNodeMsPassport::Passport::PassportResult res = CSNodeMsPassport::Passport::CreatePassportKey(
...
passport::unmanaged::passportSign(int &status, int &outSize, const char *accountId, const util::byte *challenge,
                                  int challengeSize) {
    CSNodeMsPassport::Passport::PassportResult res = CSNodeMsPassport::Passport::PassportSign(CharToString(accountId),
                                                                                              charToByteArray(challenge,
                                                                                                              challengeSize));
    status = res.status;
...
NODEMSPASSPORT_EXPORT char *passport::unmanaged::getPublicKeyHash(int &status, int &outSize, const char *accountId) {
    CSNodeMsPassport::Passport::PassportResult res = CSNodeMsPassport::Passport::GetPublicKeyHash(
            CharToString(accountId));
    status = res.status;
...
passport::unmanaged::verifyChallenge(const util::byte *challenge, int challengeSize, const util::byte *signature,
                                     int signatureSize, const util::byte *publicKey, int publicKeySize) {
    return CSNodeMsPassport::Passport::VerifyChallenge(charToByteArray(challenge, challengeSize),
...
NODEMSPASSPORT_EXPORT bool
credentials::write(const std::wstring &target, const std::wstring &user, const secure_wstring &password, bool encrypt) noexcept {
    bool ok;
...
NODEMSPASSPORT_EXPORT bool credentials::remove(const std::wstring &target) noexcept {
    return ::CredDeleteW(target.c_str(), CRED_TYPE_GENERIC, 0);
}

NODEMSPASSPORT_EXPORT bool credentials::isEncrypted(const std::wstring &target) {
    PCREDENTIALW pcred;
    bool ok;
